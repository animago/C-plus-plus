...
float wyznacz_najblizsza_sredniej(float liczby[], int ilosc)
{
    ...
    najblizsza = wartosc_bez(liczby[0]-srednia);

    for (int i=0; i<ilosc; i++)
    {
        if( wartosc_bez(liczby[i] - srednia) <= najblizsza)
        {
            najblizsza = wartosc_bez(liczby[i] - srednia);
            wybrana = liczby[i];
        }
    }

    cout << " liczba/y najblizsza sredniej: "<< wybrana;

    bool stan=false;

    for(int i=0; i<ilosc; i++)
    {
        if(wartosc_bez(liczby[i] - srednia) == najblizsza && liczby[i] != wybrana)
        {
            stan=true;
        }
